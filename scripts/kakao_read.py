...
# 채팅 row 판별용 정밀 타임스탬프 패턴. TIME_PATTERNS 의 '월'/'일'/':' 같은
# 초빈출 단글자가 마지막메시지 미리보기 본문에 박혀 과매칭(배너/공지 row 오인)되는 걸 막는다.
_TIME_RE = re.compile(
    r'(오전|오후|AM|PM)\s*\d'          # 오후 6:32 / PM 6
    r'|\d{1,2}:\d{2}'                  # 6:32
    r'|\d{1,2}월\s*\d{1,2}일'          # 6월 15일
    r'|어제|그제|그저께|Yesterday'
...
def _is_time_token(s: str) -> bool:
    """row 텍스트 토큰이 타임스탬프로 보이면 True. 미리보기/본문(긴 문자열)은 제외."""
    s = (s or "").strip()
    if len(s) > 20:  # 미리보기/메시지 본문은 타임스탬프가 아님
        return False
        ...
        for child in safe_get_attr(win, 'AXChildren', []):
            ...
            for table_child in safe_get_attr(child, 'AXChildren', []):
                ...
                for row in rows[:limit]:
                    ...
                    # 단톡방 row 는 이름 뒤에 인원수(예: "7")가 끼어 시간이 texts[1] 이 아닌
                    # texts[2+] 로 밀린다. texts[1] 만 보면 단톡방이 누락되므로 texts[1:] 전체에서 시간패턴 탐색.
                    # 단, 미리보기 본문 과매칭을 막기 위해 _is_time_token 으로 정밀 판정.
                    if len(texts) >= 2 and any(_is_time_token(tx) for tx in texts[1:]):
