...
## ⚠️ 「다른 기기에서 로그인 중 — 강제 로그아웃?」 게이트 (2026-08-27 실측)
맥 카톡에 로그인하려 하면 이 창이 뜰 때가 있다. **확인을 누르기 전에 «그 다른 기기»가 무엇인지부터 특정해라.**
- 실제 사례: 그 기기가 **아이맥(boostersui-iMac)** 이었고, 고야태스크·프라임컴퍼니·스텝레드X다산로지스 등 업무방이 붙은 **살아있는 세션**이었다. 눌렀으면 그게 끊겼다.
- 특정 방법(ssh로 됨): 후보 맥에서 `defaults read com.kakao.KakaoTalk` → `Email`(예 `821021430380`) 로 어느 계정인지, 창을 AX로 열어 채팅목록 확인.
- ⇒ **우회로가 있으면 우회해라.** 이번엔 **아이폰 미러링(iphone-mirroring-manager)** 으로 아이폰 카톡에서 처리해 PC 세션 충돌·SMS 인증·아이맥 영향을 «전부» 피했다.
- ⚠️ 단 아이폰 카톡은 **개인계정일 수 있다** — 만든 결과물의 «명의»를 끝에 반드시 확인해라(오픈채팅이면 방장이 누구인지). 오픈채팅은 관리 › **「방장 변경」** 으로 삭제 없이 명의 이전이 된다.

## ⚠️ 계정은 «사업체별»로 다르다 — 맥을 정하면 계정이 정해진다
- 스텝레드/꽃길커머스 ≠ 고야/소문의섬/고디터. 카카오 자격증명(전화번호·이메일)은 **아이디보드 시트**에 있다 (2026-08-27 실측). ⛔**이 파일은 public 레포다 — 번호·메일을 여기 적지 마라.** 값이 필요하면 시트에서 읽어라.
- 맥마다 로그인된 계정이 다르다 ⇒ **「어느 맥에서 하느냐」가 곧 「어느 계정이냐」**. 작업 맥부터 정해라.

---

## 11. ★오픈채팅 «개설» — 유형·절차·막히는 자리 (2026-09-06 실측 완주)

> 이 절이 없어서 2026-09-05 에 「8/27 에 누가 어떻게 만들었나」를 아무도 답하지 못했다.
> (지디 스킬·notes 0건 / 조조 notes·org/aar 0건 / 노션 AAR DB 는 integration 미공유라 «확인 불가»)
> ⇒ 유일한 원본이 세션 트랜스크립트 하나였다. **개설은 반드시 여기 적는다.**

### 11-1. ★유형이 «넷»이다 — 이걸 먼저 고른다
아이폰 카톡 **하단 「오픈채팅」 탭 → 우상단 «말풍선+»** → 유형 선택:

| 유형 | 무엇 | 언제 |
|---|---|---|
| **그룹채팅** | 참여자들끼리 한 방에서 대화 | 커뮤니티·모집 |
| **커뮤니티**(공개채팅) | 누구나 열람 | — |
| **1:1 채팅** | 친구 아닌 사람과 1:1 | 단발 |
| ★**오픈프로필** | 오픈채팅용 «맞춤 프로필» | ★**1:1 문의 창구는 이것** |

★★**「1:1 문의를 받는 창구」의 정답은 «오픈프로필»이다.** 오픈프로필 자체가
**참여 링크 1개 + 「최대 채팅방 수 100」** 를 갖고 있어서, 링크를 누른 **사람마다 별도 1:1 방**이 열린다.
⇒ 「1:1 채팅」 유형을 따로 만들 필요가 없다. (2026-09-06 실측으로 확정)

### 11-2. ⛔생성 «전에» 못 박히는 것 — 되돌릴 수 없다
**「기본프로필로만 참여 허용」** 토글. 화면에 그대로 적혀 있다:
> 모든 대화상대에게 적용되며 **생성 후 변경할 수 없어요.**

- **ON** = 참여자가 **카톡 기본프로필(실명)로만** 들어온다. 개설자 선택지도 «신현빈 / 카카오프렌즈» 둘뿐.
- **OFF** = 오픈프로필(닉네임)로 참여 가능.
- ★**실사고**: 8/27 방(`gzo6vNKi`)이 ON 인 채로 만들어져 **참여자가 실명으로만 입장**해야 했고,
  변경이 불가라 **9/3 에 새 방을 파야 했다**(`gm2omOLi`). ⇒ **개설 전에 이 토글부터 정해라.**

### 11-3. ⛔오픈프로필은 «최대 3개» — 꽉 차면 못 만든다
「오픈프로필은 최대 3개까지 생성이 가능합니다」 팝업이 뜬다.
⇒ 선택지는 ⑴기존 1개 **삭제**(⛔비가역·현빈 게이트) 또는 ⑵기존 1개를 **개편**(권장·되돌릴 수 있다).
★2026-09-06 엔 ⑵로 갔다 — 「효자손주식」 프로필을 「소문의섬」으로 개편. 개편 **전에 「연결된 채팅방 관리 → 목록보기」로
«붙어 있는 사람이 0명»인지 확인**했다(있으면 이름 변경이 그 사람들에게 즉시 보인다).

### 11-4. 절차 (★모바일 전용 — 맥·웹은 «불가»가 이미 확정됐다)
⛔**맥 카톡 앱·웹(open.kakao.com) 둘 다 «개설 진입점이 없다».** 2026-09-02 아이맥이 5경로 전수조사로 확정했다
(`notes/KNOWLEDGE.md` → 「맥 카톡 오픈채팅 「만들기」 — 최종 반증」). ★거기서 **srv 의 「맥에도 오픈채팅 탭이 있다」는 기술이 «반증»됐다**
— 그건 오픈채팅 탭이 아니라 **정렬 드롭다운**이었다. ⇒ 맥·웹에서 시도하지 말고 **바로 아이폰(미러링 또는 실물)** 으로 가라.

⚠️★**나(김민재)는 2026-09-06 에 이 반증을 «모른 채» 지디에게 「맥에 개설 버튼이 있었냐」를 물었다.**
내 노트가 반증됐는데 «옛 버전을 기억으로» 들고 다닌 것이다. ⇒ **행동 전에 notes 를 «다시 읽어라».** 기억은 갱신되지 않는다.

1. `iphone-mirroring-manager` 로 폰 제어. ⚠️**「iPhone 사용 중」이면 사람이 폰을 잠가야 붙는다.**
2. 카톡 → **오픈채팅 탭 → 💬+ → 오픈프로필**
3. (꽉 찼으면) **오픈채팅 관리 → 나의 오픈프로필 → 대상 카드의 «연필»**
4. 편집 화면 = 사진 / **이름** / **프로필 URL**(`open.kakao.com/me/…`, 선택) / **상태메시지** / **관심태그** / 연결된 채팅방 관리 / 오픈프로필 삭제
5. 입력 → **완료**
6. **참여 링크 확인** = 「연결된 채팅방 관리」 → **링크 정보 → 참여 링크**. 여기에 **최대 채팅방 수·검색 허용·새 채팅방 알림**도 있다.
7. ★**만든 «직후» 링크를 원장에 적어라.** 같은 이름 방이 여럿이면 나중에 어느 링크를 배포했는지 못 가린다(실제로 「소문의섬」 방이 3개가 됐다).

### 11-5. ⚠️명의 — 「어느 폰/맥이냐」가 곧 「누구 이름으로 찍히냐」
- 아이폰 미러링으로 만들면 **그 아이폰에 로그인된 계정**(= 현빈 «개인계정») 소유가 된다.
- 오픈채팅방은 관리 › **「방장 변경」** 으로 명의 이전이 된다(메뉴 실재 확인).
- ★**오픈프로필은 이름·사진을 «브랜드»로 잡으면 개인 노출이 사라진다** — 8/27 그룹방에서 닉네임이 실명으로 남았던 문제를 이걸로 막았다.

### 11-6. ⚠️검색 허용 ON = **링크를 안 뿌려도 사람이 들어온다**
실측: 링크를 아무에게도 배포하지 않은 그룹방 2개에 **각각 모르는 사람이 1명씩** 들어와 인사를 남겼다.
⇒ ★**「배포 시점」과 「문의가 들어오기 시작하는 시점」이 다르다.** 창구를 만들면 **응대 담당부터 정해라** —
「물어봤는데 답이 없는 창구」는 창구가 없는 것보다 나쁘다.

### 11-7. 막히는 자리 (미러링 조작)
`iphone-mirroring-manager` **§4-3(한/영 모드)** 와 **§4-4(막히는 자리 8건)** 를 먼저 읽어라. 요지:
- ★입력 전 **`langprobe` 로 한/영 모드를 «측정»**해라. 전환은 **`lang`(Globe/fn)** — ⛔Caps Lock·Ctrl+Space 는 안 먹는다.
- 기호(`#`)는 `type`, 한글은 `typeko` — `typeq` 는 shift 를 못 낸다(`#`→`3`).
- 프로필 «사진»을 맥에서 폰으로 못 옮긴다 → **폰 Safari 로 직접 받게** 한다(§5-B).

### 11-8. 소문의섬 링크 원장 (2026-09-06 기준 · 미제 0)
| 링크 | 유형 | 상태 |
|---|---|---|
| `open.kakao.com/o/gzo6vNKi` | 그룹 (8/27, 지디) | ⛔**폐기** — 기본프로필 강제라 변경 불가 |
| `open.kakao.com/o/gm2omOLi` | 그룹 (9/3, 대성) | **보유·재활용 대기** (트리거 = 구독자 모이면) |
| `open.kakao.com/o/spxxROHb` | ★**1:1 문의**(오픈프로필 「소문의섬」) | **운영** — 홈페이지 CTA 링크 |
- 응대 담당 = **현빈 본인**(2026-09-06 확정).
- 「최대 100」의 **101번째가 어떻게 되는지는 «모른다»** — 연결 방이 0개라 잴 수 없었다. ⛔추측으로 안내문구를 쓰지 마라.
  ⇒ **한도 감시**(연결된 방 수 주기 점검)로 가고, 90 근처에서 한도를 올리며 그때 화면을 보고 문구를 정한다. (담당 = 김민재)
