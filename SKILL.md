...
### ★★★ 전송 절대규칙 — 오후 6시 이후 답장 금지 (현빈 지시 2026-06-18)
**오후 6시(18:00) 이후**에는 외부로 나가는 **카톡 답장·전송을 절대 하지 않는다.** 6시 이후 카톡은 **읽기/다운로드/요약만**.
- 전송(`kakao_send.py`) 직전 **현재 시각(`date +%H`)이 18 이상인지 확인** → 18시 이후면 전송 중단.
- 수령 확인 답장 등 단순 응답도 6시 이후 금지.
- **예외**: 현빈이 따로 개인적으로 "보내라"고 요청한 경우에만 허용.
- 내부 보고(tele-code claude-tg, telegram-send)·쿠팡매니저 핸드오프는 '외부 답장'이 아니므로 시간 무관 허용.
- 6시 이후 답장이 필요해 보이면 보내지 말고 다음날/현빈 승인으로 보류.
